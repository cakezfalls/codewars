...
export function arrange(strng: string): string {
   ...
   for (let i = 0; i < newArray.length - 1; i++) {
    if ((i % 2 === 0 && newArray[i].length > newArray[i + 1].length) || 
        (i % 2 === 1 && newArray[i].length < newArray[i + 1].length)) {
      [newArray[i], newArray[i + 1]] = [newArray[i + 1], newArray[i]];
    }
  }
     newArray = newArray.map((word, index) => index % 2 === 0 ? word.toLowerCase() : word.toUpperCase());
